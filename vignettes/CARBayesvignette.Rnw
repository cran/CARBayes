...
\title{\pkg{CARBayes} version 4.4: An \proglang{R} Package for Spatial Areal Unit Modelling  with Conditional Autoregressive Priors}
...
\Abstract{
This is a vignette for the \proglang{R} package \pkg{CARBayes} version 4.4, and is an updated version of a paper in the Journal of Statistical Software in 2013 Volume 55 Issue 13 by the same author. This vignette describes the class of models that can be implemented in CARBayes, and gives 3 worked examples of spatial data analysis using the package. Version 4.4 has two main changes from version 4.3. Firstly, the three functions \code{S.CARiar()}, \code{S.independent()} and \code{S.CARleroux()} have been merged into \code{S.CARleroux()}, as the latter is a generalisation of the other two. This has been achieved by adding two additional arguments to \code{S.CARleroux()}, \code{fix.rho} (logical) and \code{rho} (numeric). The old model \code{S.independent()} can be obtained by setting (\code{fix.rho=TRUE, rho=0}), which corresponds to independent random effects. Similarly,  the old model \code{S.CARiar()} corresponding to the intrinsic CAR model can be obtained by setting (\code{fix.rho=TRUE, rho=1}). The second change is that the \code{modelfit} component of the  fitted model object now additionally returns the  Watanabe-Akaike Information Criterion (WAIC) and an estimate of the estimated number of effective parameters (p.w).
...
\subsection{Level 1 - Data structure and likelihood}
