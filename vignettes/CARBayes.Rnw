...
\code{S.CARdissimilarity()} &(\ref{equation binary neighbourhood}) and (\ref{equation nonbinary neighbourhood}) & fits the localised spatial autocorrelation model proposed by \cite{lee2012}. This model can be implemented with binomial, Gaussian and Poisson data likelihoods.\\\hline
...
\begin{CodeInput}
R> library(CARBayes)
R> chain1 <- S.CARleroux(formula=form, data=pricedata.sf, family="gaussian", W=W, 
+      burnin=100000, n.sample=300000, thin=100)
R> chain2 <- S.CARleroux(formula=form, data=pricedata.sf, family="gaussian", W=W, 
+      burnin=100000, n.sample=300000, thin=100)
R> chain3 <- S.CARleroux(formula=form, data=pricedata.sf, family="gaussian", W=W, 
+      burnin=100000, n.sample=300000, thin=100)
\end{CodeInput}
...
Then posterior means and 95\% credible intervals can be computed as follows:


\begin{CodeInput}
R> round(t(rbind(apply(beta.samples.matrix, 2, mean), apply(beta.samples.matrix, 
...
\begin{CodeInput}
R> boundary.coordinates <- st_coordinates(boundary.final)
R> colours <- colorNumeric(palette = "YlOrRd", domain = respiratorydata.sf$risk)
R> map3 <- leaflet(data=respiratorydata.sf) %>% 
+  addTiles() %>% 
+  addPolygons(fillColor = ~colours(risk), color="", weight=1, 
+              fillOpacity = 0.7) %>%
+  addLegend(pal = colours, values = respiratorydata.sf$risk, opacity = 1, 
+            title="Risk") %>%
