poisson.clusterCAR <- function(formula, data=NULL, G, W, burnin=0, n.sample=1000, thin=1, prior.mean.beta=NULL, prior.var.beta=NULL, prior.tau2=NULL, prior.delta = NULL, verbose=TRUE)
{
...
mod.glm <- glm(Y~X.standardised-1 + offset(offset), family=poisson)
Ytemp <- Y
Ytemp[Y==0] <- 1
res <- log(Ytemp) - offset - (X.standardised %*% mod.glm$coefficients) + mod.glm$coefficients[which(X.indicator==2)]
...
## Start timer
    ...
    #########################
    fitted <- exp(lambda[Z] + phi + beta.regression + offset)
...
fitted.median <- exp(X.standardised %*% median.beta + median.lambda[median.Z] + median.phi + offset)
