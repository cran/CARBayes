binomial.bymCAR <-
function(formula, beta=NULL, phi=NULL, theta=NULL, tau2=NULL, sigma2=NULL, trials, W, burnin=0, n.sample=1000, blocksize.beta=5, blocksize.phi=10, blocksize.theta=10, prior.mean.beta=NULL, prior.var.beta=NULL, prior.max.tau2=NULL, prior.max.sigma2=NULL)
{
##############################################
...
###########################
    ...
    proposal.beta <- beta    
        ...
        logit.proposal <- as.numeric(X.standardised %*% proposal.beta) + phi + theta + offset
        logit.current <- as.numeric(X.standardised %*% beta) + phi + theta + offset    
    ...
    proposal.phi <- phi    
        ...
        logit.proposal <- offset.temp[beg[r]:fin[r]] + proposal.phi[beg[r]:fin[r]]
        logit.current <- offset.temp[beg[r]:fin[r]] + phi[beg[r]:fin[r]]    
        prob.proposal <- exp(logit.proposal)  / (1 + exp(logit.proposal))
        prob.current <- exp(logit.current)  / (1 + exp(logit.current))
    ...
    #### Update the parameters in blocks
    offset.temp <- as.numeric(X.standardised %*% beta) + phi + offset
    proposal.theta <- theta    
        ...
        logit.proposal <- offset.temp[beg[r]:fin[r]] + proposal.theta[beg[r]:fin[r]]
        logit.current <- offset.temp[beg[r]:fin[r]] + theta[beg[r]:fin[r]]    
        prob.proposal <- exp(logit.proposal)  / (1 + exp(logit.proposal))
        prob.current <- exp(logit.current)  / (1 + exp(logit.current))
