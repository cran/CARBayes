poisson.bymCAR <-
function(formula, beta=NULL, phi=NULL, theta=NULL, tau2=NULL, sigma2=NULL, W, burnin=0, n.sample=1000, thin=1, blocksize.beta=5, blocksize.phi=10, blocksize.theta=10, prior.mean.beta=NULL, prior.var.beta=NULL, prior.max.tau2=NULL, prior.max.sigma2=NULL)
{
##############################################
...
###########################
    ...
    proposal.beta <- beta    
    phi.offset <- exp(phi + theta + offset)
    ...
    ####################
    Q.temp <- Q / tau2
    beta.offset <- exp(as.numeric(X.standardised %*% beta) + theta + offset)        
    b <- rnorm(n)
    ...
    ## Sample from theta
    ####################
    beta.offset <- exp(X.standardised %*% beta + phi + offset)
    proposal.theta <- rnorm(n, mean=theta, sd=proposal.sd.theta)
    
         for(r in 1:n.phi.block)
         {
         ## Calculate the acceptance probability
         prob1 <- sum(Y[phi.beg[r]:phi.fin[r]] * (proposal.theta[phi.beg[r]:phi.fin[r]] - theta[phi.beg[r]:phi.fin[r]]) + beta.offset[phi.beg[r]:phi.fin[r]] * (exp(theta[phi.beg[r]:phi.fin[r]]) - exp(proposal.theta[phi.beg[r]:phi.fin[r]])))
...
cat("\n\n")
cat("Acceptance rate for the random effects is  phi ", round(accept.phi,1), "%, theta ", round(accept.theta,1), "%.", "\n\n", sep="")
cat("DIC = ", DIC, "     ", "p.d = ", p.d, "\n")
