gaussian.bymCAR <-
function(formula, beta=NULL, phi=NULL, theta=NULL, nu2=NULL, tau2=NULL, sigma2=NULL, W, burnin=0, n.sample=1000, blocksize.phi=10, blocksize.theta=10, prior.mean.beta=NULL, prior.var.beta=NULL, prior.max.nu2=NULL, prior.max.tau2=NULL, prior.max.sigma2=NULL)
{
##############################################
...
summary.hyper[3, 4:5] <- c((n.sample-burnin), as.numeric(100 * (1-rejectionRate(mcmc(samples.sigma2)))))
...
fitted.temp <- array(NA, c(nrow(samples.beta), n))
    ...
    {
    fitted.temp[i, ] <- X.standardised %*% samples.beta[i, ] + samples.re[i, ] + offset
    }
