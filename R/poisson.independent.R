poisson.independent <- function(formula, data=NULL, beta=NULL, theta=NULL, sigma2=NULL, burnin=0, n.sample=1000, thin=1, blocksize.beta=5, prior.mean.beta=NULL, prior.var.beta=NULL, prior.sigma2=NULL)
{
...
CPO.temp <- array(NA, c(nrow(samples.theta), n))
    ...
    {
    temp.lp <- samples.theta[i, ]  + X.standardised %*% samples.beta[i, ] + offset
...
residuals.temp <- array(NA, c(nrow(samples.beta), n))
    ...
    {
    temp <- exp(X.standardised %*% samples.beta[i, ] + samples.theta[i, ] + offset)
    fitted.temp[i, ] <- temp
